  ...
  <div class="update_banner">
    <v-dialog v-model="dialog" width="900" persistent>
      <v-card>
        ...
        >
          {{ toUpdate ? 'Update Vehicle' : 'Add Vehicle' }}
        </v-card-title>
        <v-divider></v-divider>
        <v-divider></v-divider>
                ...
                <div class="field_container">
                  <div class="input_parent position-relative">
                    <v-autocomplete
                      ...
                      variant="outlined"
                      :error="$v.vehicleType.$error"
                      :errorMessages="$v.vehicleType.$errors[0]?.$message"
                      prependInnerIcon="mdi-at"
                      :items="vehicles.docs || []"
                      item-title="name"
                      item-value="_id"
...
const { meta, items: vehicles } = storeToRefs(vehicleStore)
...
const roles = computed(() => {
  return {
    ...
    vehicleType: {
      required: helpers.withMessage('Select vehicle', required)
    },
    phone: {
...
onMounted(() => {
  ...
  meta.value.limit = 10000
  vehicleStore.doGetItems()
  if (props.toUpdate) {
