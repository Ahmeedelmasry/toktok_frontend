...
import { userStore } from '@/stores/admin/admin'

// Validator
import useVuelidator from '@vuelidate/core'
import { required, requiredIf, helpers, email, minLength, sameAs } from '@vuelidate/validators'
...
const submitData = async () => {
  ...
  if (res) {
    ...
    if (props.toUpdate) {
      ...
      const result = await usersModule.doUpdateUser(
        {
          ...data,
          createdAt: new Date()
        },
      ...
      const result = await usersModule.doAdduser({
        ...formData.value,
        createdAt: new Date()
      })
      if (result) {
